...
static void xSemaphoreCreateCounting(struct k_sem **sem, unsigned int uxMaxCount, unsigned int uxInitialCount)
{
    if (*sem)
        k_panic();

    (*sem) = (struct k_sem *) malloc(sizeof(struct k_sem));
    if (!(*sem)) {
        printk("sem malloc failed\n");
        k_panic();
    }
    int ret = k_sem_init(*sem, uxInitialCount, uxMaxCount);
    if (ret) {
        printk("k_sem_init returned %d\n", ret);
        k_panic();
    };
}

static void xSemaphoreGive(struct k_sem *sem)
{
    k_sem_give(sem);
}

static void vSemaphoreDelete(struct k_sem *sem)
{
    if (sem)
        free(sem);
}
...
static bool xSemaphoreTake(struct k_sem *sem, k_timeout_t timeout)
{   
    int ret;
    ret = k_sem_take(sem, timeout);
    if (ret) {
        printk("Error: k_sem_take returned %d\n", ret);
        k_panic();
        return pdFALSE;
    }
    return pdTRUE;
...
static void USB_HostMsdCommandTest(usb_host_msd_command_instance_t *msdCommandInstance)
{
    ...
    {       
        xSemaphoreCreateCounting(&msdCommandInstance->commandSemaphore, 0x01U, 0x00U);
    }
    else
    {        
        vSemaphoreDelete(msdCommandInstance->commandSemaphore);
    ...
    if (pdTRUE != xSemaphoreTake(msdCommandInstance->commandSemaphore, K_FOREVER)) /* wait the command */
    {
        msd_command_test_done(msdCommandInstance);
        return;
    }
